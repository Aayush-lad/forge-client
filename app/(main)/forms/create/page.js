        ...
        <div className="md:w-1/4 p-4 border-r">
          ...
          <div className='grid grid-cols-2 gap-2'>
            {FormElementsOptions.map((item,index) => (
              <button key={index} className="bg-blue-500 text-white py-2 px-4 rounded mb-2 w-full flex flex-col items-center justify-center" title={item.title} onClick={item.onClick}>
      ...
      <div ref={setNodeRef} style={style} {...attributes} className="form-element bg-gray-100 p-4 rounded mb-2 shadow-md">
        ...
        <div className="flex items-center">
          ...
          <button
            ...
            className={`${!element.required? "bg-yellow-500":"bg-red-500"} text-white py-1 px-2 rounded ml-2`}
          >
            {element.required ? 'Make Field Optional' : 'Make Field Required'}
          </button>
        </div>
      </div>
    );
